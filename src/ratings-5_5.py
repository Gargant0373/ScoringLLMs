...
from schwartz import ValueInformation, RubricInformation, schwartz_values, negative_rubric
...
pattern = re.compile(r'ratings_negative-\d+\.csv')
count = len([name for name in os.listdir(results_dir) if pattern.match(name)])

result_file_path = os.path.join(results_dir, f"ratings_negative-{count}.csv")
weighted_file_path = os.path.join(results_dir, f"weighted_negative-{count}.csv")
...
class OutputScore(BaseModel):
    score: int = Field(
        ge=-5,
        le=5,
        description="An integer between -5 and 5. Refer to the score rubric"
    )
...
class GenerateScore(dspy.Signature):
    ...
    integer in the interval [-5, 5], according to the rubric, based on how important
